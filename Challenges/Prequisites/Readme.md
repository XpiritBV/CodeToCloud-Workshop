...
Visual Studio Code doesn't pass a number of keystrokes to the terminal, this may make it impossible to quit docker once you've started a container. You can add the [custom keybindings specified here](/.devcontainer/keybindings.json) to [your Visual Studio Code settings](https://code.visualstudio.com/docs/getstarted/keybindings#_advanced-customization). This remaps `ctrl-q` and `ctrl-p` when the terminal has focus.
